...
function Header() {
  ...
  return (
    <header className={`relative w-full h-[500px] ${headerBgClass}`}>
      ...
      <div className="absolute left-[74%] top-6 -translate-x-1/2 flex items-center gap-3 text-lg font-bold">
        <button
          ...
          className={`flex items-center gap-2 rounded-full px-3 py-1 text-xs transition ${darkMode ? 'text-white' : 'text-[#3730A3]'}`}
        >
          <span className={`flex h-5 w-10 items-center rounded-full ${darkMode ? 'bg-[#94A3B8]/30' :
            'bg-[#3730A3]/70'} p-0.5 transition ${darkMode ? 'justify-end' : 'justify-start'}`}>
